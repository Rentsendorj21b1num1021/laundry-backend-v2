...
export const auth = async (req, res, next) => {
  try {
    ...
    if (!user?.isActive) {
      return res.status(401).json({ message: 'Хэрэглэгч олдсонгүй эсвэл идэвхигүй байна' })
    }
    ...
    if (err.name === 'TokenExpiredError') {
      return res.status(401).json({ message: 'Token-ий хугацаа дууссан байна' })
    }
...
export const requireOrganization = async (req, res, next) => {
  try {
    ...
    const orgId = req.headers['x-organization-id'] || req.user.defaultOrganization
    ...
    const hasAccess = req.user.organizations.some((org) => org.organizationId.toString() === orgId.toString() && org.isActive)
    ...
    const userOrgAccess = req.user.organizations.find((org) => org.organizationId.toString() === orgId.toString())
...
export const requireOrgRole = (...allowedRoles) => {
  return (req, res, next) => {
    ...
    if (!req.userOrgRole) {
      return res.status(403).json({ message: 'Та энэ байгууллагад хандах эрхгүй' })
    }
...
export const canCreateOrganization = async (req, res, next) => {
  try {
    ...
    return res.status(403).json({
      message: 'Байгууллага үүсгэх эрхгүй байна. Зөвхөн owner эсвэл manager шинэ байгууллага үүсгэж болно.',
