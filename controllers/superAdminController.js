...
export const getAllOrganizations = async (req, res) => {
  try {
    ...
    const [organizations, total] = await Promise.all([
      Organization.find(filter).populate('ownerId', 'username email phone').sort({ createdAt: -1 }).skip(skip).limit(parseInt(limit, 10)),
...
export const getAllUsers = async (req, res) => {
  try {
    ...
    if (search) {
      filter.$or = [{ username: { $regex: search, $options: 'i' } }, { email: { $regex: search, $options: 'i' } }, { phone: { $regex: search, $options: 'i' } }]
    }

    const skip = (parseInt(page, 10) - 1) * parseInt(limit, 10)

    const [users, total] = await Promise.all([
      User.find(filter).select('-passwordHash').populate('organizations.organizationId', 'name').sort({ createdAt: -1 }).skip(skip).limit(parseInt(limit, 10)),
