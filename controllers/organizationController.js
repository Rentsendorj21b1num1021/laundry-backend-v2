...
export const createOrganization = async (req, res) => {
  try {
    const { name, address, phone, email, bonusPercentage, orderPrefix, businessType } = req.body
...
export const getUserOrganizations = async (req, res) => {
  try {
    ...
    const activeOrganizations = user.organizations.filter((org) => org.isActive && org.organizationId?.status === 'active')
...
export const updateOrganizationSettings = async (req, res) => {
  try {
    ...
    const organization = await Organization.findByIdAndUpdate(organizationId, { $set: updates }, { new: true, runValidators: true })
